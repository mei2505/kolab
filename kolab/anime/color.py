...
class ColorTheme(object):
    ...
    def __init__(self, name=None):
        if name in ColorThemeDataSet:
            self.colors = ColorThemeDataSet[name]
        else:
            keys = list(ColorThemeDataSet.keys())
            random.shuffle(keys)
            self.colors = ColorThemeDataSet[keys[0]]
    
    def color(self, n=None):
        if n is None:
